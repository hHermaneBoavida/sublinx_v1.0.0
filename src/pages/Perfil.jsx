...
import {
  ...
  Settings, Shield, Music, Clock, Heart, MessageCircle, Trophy, Users
} from "lucide-react";
  ...
  const { data: recentActivity = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [likes, comments] = await Promise.all([
        base44.entities.Like.filter({ user_id: user.id }, "-created_date", 10),
        base44.entities.Comment.filter({ user_id: user.id }, "-created_date", 10)
      ]);
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <Tabs defaultValue={user.is_organizer ? "eventos" : "ingressos"} className="w-full">
          <TabsList className="w-full grid grid-cols-4 bg-black border-b border-gray-800 rounded-none h-auto p-0">
            <TabsTrigger 
              value={user.is_organizer ? "eventos" : "ingressos"}
            ...
            {user.is_organizer ? (
              myEvents.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <Button onClick={() => navigate(createPageUrl("CriarEvento"))} className="bg-cyan-600 hover:bg-cyan-700">
                    Criar Evento
                  </Button>
                </div>
              )
      ...
      {showLogoutConfirm && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-xl font-bold text-white text-center mb-2">Sair da conta?</h2>
